...
export const getDashboardStats = async () => {
    ...
    // 1. Today's Sales
    const todaySalesRes = await query(`SELECT COALESCE(SUM(final_total), 0) as total 
         FROM sales 
         WHERE date(created_at) = $1 AND status != 'Cancelled'`, [today]);
    const todaySales = parseFloat(todaySalesRes.rows[0]?.total || 0);
    // 2. Month's Sales
    // Postgres: to_char(created_at, 'YYYY-MM')
    // SQLite: strftime('%Y-%m', created_at)
    const monthSalesRes = await query(`SELECT COALESCE(SUM(final_total), 0) as total 
         FROM sales 
    ...
    // 4. Ongoing Repairs
    const repairsRes = await query(`SELECT COUNT(*) as count 
         FROM repairs 
         WHERE status IN ('reçue', 'en_cours', 'en_attente_pieces')`);
    const ongoingRepairs = parseInt(repairsRes.rows[0]?.count || 0);
    // 5. Low Stock
    const lowStockRes = await query(`SELECT COUNT(*) as count FROM products WHERE stock_quantity <= 5`);
    const lowStockCount = parseInt(lowStockRes.rows[0]?.count || 0);
    // 6. Recent Repairs (Top 5)
    // Client name join
    const recentRepairsRes = await query(`SELECT r.*, c.name as client_name 
         FROM repairs r
         LEFT JOIN clients c ON r.client_id = c.id
         ORDER BY r.created_at DESC 
         LIMIT 5`);
    // 7. Low Stock Items (List)
    const lowStockItemsRes = await query(`SELECT * FROM products WHERE stock_quantity <= 5 LIMIT 5`);
    return {
        todaySales,
        monthSales,
        todayProfit,
    ...
    const queryStr = `
        ...
        SalesStats AS (
            SELECT 
                ...
                SUM(final_total * 0.45) as profits
            FROM sales
